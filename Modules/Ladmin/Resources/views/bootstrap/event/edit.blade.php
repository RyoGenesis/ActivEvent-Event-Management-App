<x-ladmin-auth-layout>
    ...
    <form action="{{ route('ladmin.event.update', $event->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        {{-- name --}}
        ...
        <div class="row d-flex mb-3">
            ...
            <div class="col">
                <textarea name="description" id="description" class="form-control" rows="8" placeholder="Event detail and description...">{!!$event->description!!}</textarea>
                @error('description')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
        ...
        <div class="row d-flex align-items-center mb-3">
            ...
            <div class="col">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="1" id="has_certificate" name="has_certificate" aria-label="Provide certificate" {{$event->has_certificate ? 'checked' : ''}}>
                </div>
        ...
        <div class="row d-flex align-items-center mb-3">
            ...
            <div class="col">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="1" id="has_comserv" name="has_comserv" aria-label="Provide community service hour" {{$event->has_comserv ? 'checked' : ''}}>
                </div>
                @error('has_comserv')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
        ...
        <div class="row d-flex mb-3">
            ...
            <div class="col">
                <div class="form-check mb-3">
                    <input class="form-check-input" type="checkbox" value="1" id="has_sat" name="has_sat" {{$event->has_sat ? 'checked' : ''}}>
                    <label class="form-check-label" for="has_sat">
                        Provide SAT
                    </label>
                </div>
                <select name="sat_level_id" id="sat_level_id" data-placeholder="Select SAT Level" class="form-select form-control @error('sat_level_id') is-invalid @enderror" {{!$event->has_sat ? 'disabled' : ''}}>
                    <option></option>
                    @foreach ($sat_levels as $level)
                        <option value="{{$level->id}}" {{ $level->id == $event->sat_level_id ? 'selected' : '' }}>{{ $level->name }}</option>
                    @endforeach
                </select>
                @error('sat_level_id')
                    <small class="text-danger">{{ $message }}</small>
        ...
        <div class="row d-flex mb-3">
            ...
            <div class="col">
                <select name="bgas[]" id="bgas" data-placeholder="Select BGA" class="form-select form-control @error('bgas') is-invalid @enderror" multiple {{!$event->has_sat ? 'disabled' : ''}}>
                    @foreach ($bgas as $bga)
                        <option value="{{$bga->id}}" {{ in_array($bga->id, $eventBgas) ? 'selected' : '' }}>{{ $bga->name }}</option>
                    @endforeach
                </select>
                @error('bgas')
                    <small class="text-danger">{{ $message }}</small>
            ...
            <div class="col">
                <select name="majors[]" id="majors" data-placeholder="Select majors" class="form-select form-control @error('majors') is-invalid @enderror" multiple {{$event->status == 'Active' ? 'disabled' : ''}}>
                    @foreach ($majors as $major)
                        <option value="{{$major->id}}" {{ in_array($major->id, $eventMajors) ? 'selected' : '' }}>{{ $major->name }}</option>
                    @endforeach
                </select>
                @error('majors')
                    <small class="text-danger">{{ $message }}</small>
